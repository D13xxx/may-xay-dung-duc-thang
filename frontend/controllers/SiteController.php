...
class SiteController extends Controller
{
    ...
    public function actionIndex()
    {
        ...
        $sanPhamNoiBatKhuyenMai = Products::find()->where(['type'=>Products::T_SANPHAMKHUYENMAI])->andWhere(['status'=>Products::IS_ACTIVE])->orderBy(['id'=>SORT_DESC])->one();

        $sanPhamKhuyenMai = Products::find()->where(['type'=>Products::T_SANPHAMKHUYENMAI])->andWhere(['status'=>Products::IS_ACTIVE])->orderBy(['rand()'=>SORT_DESC])->limit(6)->all();
        //Danh sách sản phẩm bán chạy limit 8
        //Danh sách sản phẩm bán chạy nội bật limit 1 random
        $sanPhamNoiBatBanChay = Products::find()->where(['type'=>Products::T_SANPHAMBANCHAY])->andWhere(['status'=>Products::IS_ACTIVE])->orderBy(['id'=>SORT_DESC])->one();
        $sanPhamBanChay = Products::find()->where(['type'=>Products::T_SANPHAMBANCHAY])->andWhere(['status'=>Products::IS_ACTIVE])->orderBy(['rand()'=>SORT_DESC])->limit(6)->all();
        //Danh sách sản phẩm mới limit 8
        //Danh sách sản phẩm mới nội bật limit 1 random
        $sanPhamNoiBatMoi = Products::find()->where(['type'=>Products::T_SANPHAMMOI])->andWhere(['status'=>Products::IS_ACTIVE])->orderBy(['id'=>SORT_DESC])->one();
        $sanPhamMoi = Products::find()->where(['type'=>Products::T_SANPHAMMOI])->andWhere(['status'=>Products::IS_ACTIVE])->orderBy(['rand()'=>SORT_DESC])->limit(6)->all();
        //Danh sách nhóm sản phẩm parent_id = 0
        $listCat = CatProducts::find()->where(['parent_id'=>null])->orderBy(['sort'=>SORT_ASC])->all();
        // danh sách slide
    ...
    public function actionError(){
     ...
     if ( !empty($_GET) ) {
       ...
       $rating->table =  $_GET['table'];
       $rating->star =  $_GET['star'];
       $rating->post_id =  $_GET['post_id'];
       $rating->created_at = date('Y-m-d H:i:s');
       $rating->client_ip = $this->get_client_ip();

       $session = $this->get_client_ip().'-'.$_GET['post_id'];
       if($session != Yii::$app->session->get('client_ip')) {
           if($rating->save()){
               Yii::$app->session->set('client_ip', $this->get_client_ip().'-'.$_GET['post_id']);
               echo 'ok';
           }
       }
