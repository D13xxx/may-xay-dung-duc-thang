...
class LoginForm extends Model
{
    ...
    {
        return [
            'identity' => Yii::t('frontend', 'Mã cán bộ'),
            'password' => Yii::t('frontend', 'Mật khẩu'),
            'rememberMe' => Yii::t('frontend', 'Ghi nhớ'),
        ];
    }
    ...
    public function validatePassword()
    {
        if (!$this->hasErrors()) {
            ...
            if (!$user || !$user->validatePassword($this->password)) {
                $this->addError('password', Yii::t('frontend', 'Không đúng mật khẩu, mã cán bộ đăng nhập.'));
            }
        }
    }
