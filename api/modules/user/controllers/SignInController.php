...
use api\modules\user\models\PasswordResetRequestForm;
use api\modules\user\models\ResetPasswordForm;
...
class SignInController extends \yii\web\Controller
{
    ...
    public function actionLoginByPass($token)
    {
        ...
        $user = CanBoUserToken::getUser($token, CanBoUserToken::TYPE_LOGIN_PASS);
    ...
    {
        $token = CanBoUserToken::find()
            ->byType(CanBoUserToken::TYPE_ACTIVATION)
            ->byToken($token)
            ->notExpired()
            ->one();
    ...
    public function successOAuthCallback($client)
    {
        ...
        if (!$user) {
            ...
            $user->status = CanBoUser::STATUS_ACTIVE;
            ...
            if ($user->save()) {
                ...
                if ($user->email && CanBoUser::find()->where(['email' => $user->email])->count()) {
                    Yii::$app->session->setFlash(
                        'alert',
                        [
                            'options' => ['class' => 'alert-danger'],
